...
import { FIELD_DIRECTORY_NAME, MASTER_DETAIL_TAG, OBJECT_TYPE, } from '../constant/metadataConstants.js';
...
export default class CustomObjectHandler extends StandardHandler {
    ...
    async _handleMasterDetailException() {
        if (this.metadataDef.xmlName !== OBJECT_TYPE)
            return;
        ...
        const masterDetailsFields = await asyncFilter(fields, async (path) => {
            const content = await readPathFromGit({ path, oid: this.config.to }, this.config);
...
__decorate([
    log
], CustomObjectHandler.prototype, "handleAddition", null);
