...
import { getLatestSupportedVersion, isVersionSupported, } from '../metadata/metadataManager.js';
...
export default class CLIHelper {
    ...
    async _validateGitSha() {
        const errors = [];
        await Promise.all([FROM, TO].map(async (shaParameter) => {
            const shaValue = this.config[shaParameter];
            try {
                const ref = await this.gitAdapter.parseRev(shaValue);
                this.config[shaParameter] = ref;
            }
            catch {
    ...
    _apiVersionDefault() {
        ...
        if (!isInputVersionSupported) {
            ...
            if (this.config.apiVersion !== undefined &&
                ...
                this.work.warnings.push(new Error(this.message.getMessage('warning.ApiVersionNotSupported', [
...
__decorate([
    log
], CLIHelper.prototype, "validateConfig", null);
