    ...
    handleSave(event){
        ...
        for(var i =0; i < selected.length;i++){ 
            ...
            if(index > -1 ){
                ...
                if(this.draftValues[i].Fee_Item_Description__c != this.draftValues[index].Fee_Item_Code__c){
                    selected[i].Fee_Item_Description__c = this.draftValues[index].Fee_Item_Code__c;
                }
                if(this.draftValues[i].Fee_Item_Code__c != this.draftValues[index].Fee_Item_Description__c){
                    selected[i].Fee_Item_Code__c = this.draftValues[index].Fee_Item_Description__c;
                }
                if(this.draftValues[i].Practitioner_Name__c != this.draftValues[index].Practitioner_Number__c){
                ...
                if(this.draftValues[i].Diagnostic_Code__c != this.draftValues[index].Diagnostic_Code__c){
                    selected[i].Diagnostic_Code__c = this.draftValues[index].Diagnostic_Code__c;
                }
                if(this.draftValues[i].Diagnostic_Description__c != this.draftValues[index].Diagnostic_Description__c){
                    selected[i].Diagnostic_Description__c = this.draftValues[index].Diagnostic_Description__c;
                }
                if(this.draftValues[i].Amount_Paid__c != this.draftValues[index].Amount_Paid__c){
                    selected[i].Amount_Paid__c = this.draftValues[index].Amount_Paid__c;
                }
                if(this.draftValues[i].Total_Cost_Override__c != this.draftValues[index].Total_Cost_Override__c){
                    selected[i].Total_Cost_Override__c = this.draftValues[index].Total_Cost_Override__c;
                }
                if(this.draftValues[i].Specialty_Code__c != this.draftValues[index].Specialty_Code__c){
                    selected[i].Specialty_Code__c = this.draftValues[index].Specialty_Code__c;
                }
                if(this.draftValues[i].Specialty_Description2__c != this.draftValues[index].Specialty_Description2__c) {
                    selected[i].Specialty_Description2__c = this.draftValues[index].Specialty_Description2__c;
                }
                if(this.draftValues[i].Payee_Number__c != this.draftValues[index].Payee_Number__c){
                    selected[i].Payee_Number__c = this.draftValues[index].Payee_Number__c;
                }
                if(this.draftValues[i].Payee_Description__c != this.draftValues[index].Payee_Description__c){
                    selected[i].Payee_Description__c = this.draftValues[index].Payee_Description__c;
                }
                if(this.draftValues[i].Service_Start_Date__c != this.draftValues[index].Service_Start_Date__c){
                    selected[i].Service_Start_Date__c = this.draftValues[index].Service_Start_Date__c;
                }
                if(this.draftValues[i].Service_Finish_Date__c != this.draftValues[index].Service_Finish_Date__c){
                    selected[i].Service_Finish_Date__c = this.draftValues[index].Service_Finish_Date__c;
                }
                if(this.draftValues[i].Location_Type_Code__c != this.draftValues[index].Location_Type_Code__c){
                    selected[i].Location_Type_Code__c = this.draftValues[index].Location_Type_Code__c;
                }
                if(this.draftValues[i].Location_Type_Description2__c != this.draftValues[index].Location_Type_Description2__c){
